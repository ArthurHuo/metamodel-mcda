 ...
 * </ul>
 */
public class McdaBracketPreferencePage extends org.eclipse.jface.preference.PreferencePage implements org.eclipse.ui.IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = mcda.demo1.resource.mcda.ui.McdaPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new mcda.demo1.resource.mcda.mopp.McdaMetaInformation().getSyntaxName();
	
	private mcda.demo1.resource.mcda.ui.McdaBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public McdaBracketPreferencePage() {
		super();
		
		mcda.demo1.resource.mcda.IMcdaMetaInformation metaInformation = new mcda.demo1.resource.mcda.mopp.McdaMetaInformation();
	...
	public void init(org.eclipse.ui.IWorkbench workbench) {
		setPreferenceStore(mcda.demo1.resource.mcda.ui.McdaUIPlugin.getDefault().getPreferenceStore());
		setDescription("Define the coloring of matching brackets.");
		
		bracketsTmp = new mcda.demo1.resource.mcda.ui.McdaBracketSet(null, null);
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + mcda.demo1.resource.mcda.ui.McdaPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(		mcda.demo1.resource.mcda.ui.McdaPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		mcda.demo1.resource.mcda.ui.McdaPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.setBrackets(getPreferenceStore().getString(language + mcda.demo1.resource.mcda.ui.McdaPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		...
		bracketsList.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int index : itemIndices) {
					mcda.demo1.resource.mcda.IMcdaBracketPair bracketPair = bracketsTmp.getBracketPair(index);
		...
		enableClosingInside.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int idx : itemIndices) {
					mcda.demo1.resource.mcda.IMcdaBracketPair bracketPair = bracketsTmp.getBracketPair(idx);
	...
	protected void performDefaults() {
		enableCheckbox.setSelection(getPreferenceStore().getDefaultBoolean(mcda.demo1.resource.mcda.ui.McdaPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		...
		bracketSetTemp.put(language, getPreferenceStore().getDefaultString(language + mcda.demo1.resource.mcda.ui.McdaPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		bracketsTmp.setBrackets(bracketSetTemp.get(language));
		bracketsList.setItems(bracketsTmp.getBracketArray());
		enableClosingInside.setSelection(false);
	}
	...
	private void updateActiveEditor() {
		...
		getPreferenceStore().setValue(mcda.demo1.resource.mcda.ui.McdaPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		getPreferenceStore().setValue(language + mcda.demo1.resource.mcda.ui.McdaPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		...
		if (editor != null && editor instanceof mcda.demo1.resource.mcda.ui.McdaEditor) {
			((mcda.demo1.resource.mcda.ui.McdaEditor) editor).invalidateTextRepresentation();
		}
	}
