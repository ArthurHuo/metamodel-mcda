 ...
 */
public class McdaOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "mcda.demo1.resource.mcda.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "mcda.demo1.resource.mcda.ui.occurences.declaration";
	...
	private mcda.demo1.resource.mcda.ui.McdaTokenScanner tokenScanner;
	...
	private mcda.demo1.resource.mcda.IMcdaTextResource textResource;
	 ...
	 */
	public McdaOccurrence(mcda.demo1.resource.mcda.IMcdaTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, mcda.demo1.resource.mcda.ui.McdaTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new mcda.demo1.resource.mcda.mopp.McdaMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(mcda.demo1.resource.mcda.ui.McdaBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, mcda.demo1.resource.mcda.ui.McdaPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, mcda.demo1.resource.mcda.ui.McdaPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, mcda.demo1.resource.mcda.ui.McdaPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == mcda.demo1.resource.mcda.ui.McdaPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(mcda.demo1.resource.mcda.ui.McdaOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(mcda.demo1.resource.mcda.ui.McdaOccurrence.DECLARATION_ANNOTATION_ID);
	}
