...
public abstract class AbstractMcdaDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<mcda.demo1.resource.mcda.util.McdaPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<mcda.demo1.resource.mcda.util.McdaPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			new mcda.demo1.resource.mcda.util.McdaRuntimeUtil().logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			new mcda.demo1.resource.mcda.util.McdaRuntimeUtil().logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			mcda.demo1.resource.mcda.debug.McdaDebugMessage message = new mcda.demo1.resource.mcda.debug.McdaDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				mcda.demo1.resource.mcda.util.McdaPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new mcda.demo1.resource.mcda.util.McdaPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new mcda.demo1.resource.mcda.util.McdaPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.STARTED, true);
			setSuspend(true);
