...
public class McdaDebugTarget extends mcda.demo1.resource.mcda.debug.McdaDebugElement implements org.eclipse.debug.core.model.IDebugTarget, mcda.demo1.resource.mcda.debug.IMcdaDebugEventListener {
	
	/**
	 * associated system process
	 */
	private mcda.demo1.resource.mcda.debug.McdaDebugProcess process;
	...
	private mcda.demo1.resource.mcda.debug.McdaDebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<mcda.demo1.resource.mcda.debug.IMcdaDebugEventListener> eventListeners = new java.util.ArrayList<mcda.demo1.resource.mcda.debug.IMcdaDebugEventListener>();
	
	private mcda.demo1.resource.mcda.debug.McdaDebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new mcda.demo1.resource.mcda.mopp.McdaMetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				mcda.demo1.resource.mcda.debug.McdaDebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(mcda.demo1.resource.mcda.debug.McdaDebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((mcda.demo1.resource.mcda.debug.IMcdaDebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public McdaDebugTarget(mcda.demo1.resource.mcda.debug.McdaDebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new mcda.demo1.resource.mcda.debug.McdaDebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new mcda.demo1.resource.mcda.debug.McdaDebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(mcda.demo1.resource.mcda.debug.IMcdaDebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(mcda.demo1.resource.mcda.debug.IMcdaDebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new mcda.demo1.resource.mcda.mopp.McdaMetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					mcda.demo1.resource.mcda.debug.McdaLineBreakpoint lineBreakpoint = (mcda.demo1.resource.mcda.debug.McdaLineBreakpoint) breakpoint;
	...
	public void handleMessage(mcda.demo1.resource.mcda.debug.McdaDebugMessage message) {
		try {
			if (message.hasType(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.STARTED)) {
				started();
			} else if (message.hasType(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.SUSPENDED)) {
				suspend();
			} else if (message.hasType(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.TERMINATED)) {
				terminated();
			} else if (message.hasType(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.RESUMED)) {
	...
	public mcda.demo1.resource.mcda.debug.McdaDebugProxy getDebugProxy() {
