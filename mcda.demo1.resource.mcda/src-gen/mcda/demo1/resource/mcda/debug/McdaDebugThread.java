...
public class McdaDebugThread extends mcda.demo1.resource.mcda.debug.McdaDebugElement implements org.eclipse.debug.core.model.IThread, mcda.demo1.resource.mcda.debug.IMcdaDebugEventListener {
	
	private boolean suspended = false;
	private mcda.demo1.resource.mcda.debug.McdaDebugTarget debugTarget;
	
	public McdaDebugThread(mcda.demo1.resource.mcda.debug.McdaDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			mcda.demo1.resource.mcda.debug.McdaDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = mcda.demo1.resource.mcda.util.McdaStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new mcda.demo1.resource.mcda.debug.McdaStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public mcda.demo1.resource.mcda.debug.McdaDebugTarget getTarget() {
		return debugTarget;
	}
