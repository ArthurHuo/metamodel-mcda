 ...
 */
public class McdaDebuggableInterpreter<ResultType, ContextType> extends mcda.demo1.resource.mcda.debug.AbstractMcdaDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private mcda.demo1.resource.mcda.util.AbstractMcdaInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private mcda.demo1.resource.mcda.IMcdaCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public McdaDebuggableInterpreter(mcda.demo1.resource.mcda.util.AbstractMcdaInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new mcda.demo1.resource.mcda.IMcdaInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = mcda.demo1.resource.mcda.util.McdaStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public mcda.demo1.resource.mcda.util.AbstractMcdaInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		mcda.demo1.resource.mcda.IMcdaLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private mcda.demo1.resource.mcda.IMcdaLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		mcda.demo1.resource.mcda.IMcdaCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = mcda.demo1.resource.mcda.util.McdaEObjectUtil.getDepth(current);
		stopCondition = new mcda.demo1.resource.mcda.IMcdaCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = mcda.demo1.resource.mcda.util.McdaEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = mcda.demo1.resource.mcda.util.McdaEObjectUtil.getDepth(current) - 1;
		stopCondition = new mcda.demo1.resource.mcda.IMcdaCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
