 ...
 * code).
 */
public class McdaDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private mcda.demo1.resource.mcda.debug.AbstractMcdaDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, mcda.demo1.resource.mcda.util.McdaPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, mcda.demo1.resource.mcda.util.McdaPair<String, Object>>();
	...
	public McdaDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		mcda.demo1.resource.mcda.debug.McdaDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = mcda.demo1.resource.mcda.util.McdaStringUtil.encode('#', stack);
				mcda.demo1.resource.mcda.debug.McdaDebugMessage message = new mcda.demo1.resource.mcda.debug.McdaDebugMessage(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				mcda.demo1.resource.mcda.debug.McdaDebugMessage message = new mcda.demo1.resource.mcda.debug.McdaDebugMessage(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.GET_VARIABLES)) {
				...
				mcda.demo1.resource.mcda.debug.McdaDebugMessage message = new mcda.demo1.resource.mcda.debug.McdaDebugMessage(mcda.demo1.resource.mcda.debug.EMcdaDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return mcda.demo1.resource.mcda.util.McdaStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		mcda.demo1.resource.mcda.util.McdaPair<String, Object> pair = new mcda.demo1.resource.mcda.util.McdaPair<String, Object>(name, value);
	...
	public mcda.demo1.resource.mcda.debug.AbstractMcdaDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(mcda.demo1.resource.mcda.debug.AbstractMcdaDebuggable debuggable) {
