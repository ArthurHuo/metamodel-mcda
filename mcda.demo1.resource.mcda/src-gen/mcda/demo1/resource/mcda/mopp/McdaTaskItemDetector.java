...
/**
 * The McdaTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * McdaTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class McdaTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<mcda.demo1.resource.mcda.mopp.McdaTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<mcda.demo1.resource.mcda.mopp.McdaTaskItem> foundItems = new java.util.ArrayList<mcda.demo1.resource.mcda.mopp.McdaTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new mcda.demo1.resource.mcda.mopp.McdaTaskItem(keyword, message, localLine, offset, end));
