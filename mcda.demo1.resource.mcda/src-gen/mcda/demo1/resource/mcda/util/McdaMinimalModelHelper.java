 ...
 */
public class McdaMinimalModelHelper {
	
	private final static mcda.demo1.resource.mcda.util.McdaEClassUtil eClassUtil = new mcda.demo1.resource.mcda.util.McdaEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + mcda.demo1.resource.mcda.util.McdaStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = mcda.demo1.resource.mcda.util.McdaListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + mcda.demo1.resource.mcda.util.McdaStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = mcda.demo1.resource.mcda.util.McdaListUtil.castListUnchecked(value);
